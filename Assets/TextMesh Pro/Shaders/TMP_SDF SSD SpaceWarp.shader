Shader "TextMeshPro/Distance Field SSD - SpaceWarp"
{
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            float4 PixShaderMotion(pixel_t input) : SV_Target
            {
                ...
                #if UNDERLAY_ON
                float d = tex2D(_MainTex, input.texcoord2.xy).a * bScale;
                faceColor += float4(_UnderlayColor.rgb * _UnderlayColor.a, _UnderlayColor.a) * saturate(d - bBias) * (1
                    - faceColor.a);
                #endif

                #if UNDERLAY_INNER
                float d = tex2D(_MainTex, input.texcoord2.xy).a * bScale;
                faceColor += float4(_UnderlayColor.rgb * _UnderlayColor.a, _UnderlayColor.a) * (1 - saturate(d - bBias))
